...
public class DBConstants {
    ...
    //Dedicated for test
    public static final String GET_TICKET_TEST = "select t.PARKING_NUMBER, t.ID, t.PRICE, t.IN_TIME, t.OUT_TIME, p.TYPE from ticket t, parking p where p.parking_number = t.parking_number and t.VEHICLE_REG_NUMBER=? ORDER BY OUT_TIME DESC LIMIT 1";
    ...
    public static final String GET_RECURRENT_VEHICLE = "select count(*) as nb_ticket from ticket where ticket.VEHICLE_REG_NUMBER = ? and ticket.OUT_TIME is not NULL and datediff(IN_TIME,CURRENT_TIMESTAMP) <= ?";
     ...
     //check if vehicle is in the parkings
